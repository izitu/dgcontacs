...
sheet = wb.active
my_file = open("import-rodit1.csv", "w", encoding='utf-8')
...
for i in range(3, rows + 1):
    fio = str(sheet.cell(row=i, column=5).value).split()
    tel = str(sheet.cell(row=i, column=6).value)
    mail = str(sheet.cell(row=i, column=7).value)
